...
- **必须放真实截图时保持单文件**：图片先压到长边 ≤1600px，再以 base64 data URI 内嵌。产物仍是一个文件，拖到任何机器任何目录都能开，不会因为换了位置图挂掉。
...
**页内布局用了 vh / vw 的 deck，打印前必须换容器查询单位**。vh / vw 永远跟视口走：屏幕端 slide 铺满视口时没问题，打印端舞台被 zoom 成固定尺寸后，vh 还按纸张视口算，版面直接崩。解法：

```css
.slide{ container-type:size }   /* slide 自己当尺寸容器 */
/* 页内所有 vh→cqh、vw→cqw，屏幕端表现不变，打印端跟随 slide 实际尺寸 */
```

生成器里一句正则收口（手写 deck 就全局替换）：

```python
CSS = re.sub(r'(?<=[\d.])vh(?![a-z])', 'cqh', CSS)
CSS = re.sub(r'(?<=[\d.])vw(?![a-z])', 'cqw', CSS)
```

body 级覆盖层（工具栏 / 进度条 / lightbox / 页码）没有尺寸容器祖先，cq 单位自动回退视口，行为不变，不用单独处理。这套已在一份 105 页混排 deck 上验证：屏幕像素级一致，打印 105 页干净横版。
